...
std::vector<std::string> parseNestedList(std::string const &s, char delim = ',', char lbracket = '[',
                                         char rbracket = ']') {
...
bool isList(std::string const &s) { return s.find('[') != std::string::npos; }

bool isInteger(std::string const &s) { return !s.empty() && std::all_of(std::begin(s), std::end(s), ::isdigit); }

enum Order { CORRECT = 1, INCORRECT = -1, UNDECIDED = 0 };
...
int main() {
    ...
    std::for_each(std::begin(packets), std::end(packets), [&allPackets](auto const &p) {
        allPackets.insert(p.first);
        allPackets.insert(p.second);
    });
