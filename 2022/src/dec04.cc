 ...
 *
 */
bool doesFullyContain(Range x, Range y) { return (x.min <= y.min) && (x.max >= y.max); }
...
int main() {
    ...
    const auto totalOverlap = std::count_if(std::begin(values), std::end(values),
                                            [](auto const &x) -> bool { return doesOverlap(x.first, x.second); });
