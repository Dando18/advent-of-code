...
uint64_t getSize(File &f, bool setDirs = false) {
    ...
    uint64_t sum = 0;
    for (auto& child : f.children) sum += getSize(child, setDirs);
...
int main() {
    ...
    while (!toVisit.empty()) {
        ...
        if (curFile->isDir) {
            ...
            for (auto& child : curFile->children) toVisit.push(&child);
        }
    }
    std::cout << sum << "\n";
    ...
    while (!toVisit.empty()) toVisit.pop(); /* clear */
    toVisit.push(&root);
    while (!toVisit.empty()) {
        auto curFile = toVisit.top();
        toVisit.pop();
