...
char getSharedItem(std::string const &a, std::string const &b) {
    auto it = std::find_if(std::begin(a), std::end(a), [&b](auto const &c) { return b.find(c) != std::string::npos; });
    return *it;
}
...
char getSharedItem(std::vector<std::string> strs) {
    ...
    auto it = std::find_if(std::begin(first), std::end(first), [&strs](auto const &c) {
        return std::all_of(std::next(std::begin(strs), 1), std::end(strs),
                           [&c](auto const &s) { return s.find(c) != std::string::npos; });
    });
    return *it;
}
